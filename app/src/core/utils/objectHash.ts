...
interface ObjectHashTableItem {
    value: any;
    count: number;
}

interface ObjectHashTableItemWithKey extends ObjectHashTableItem {
    hash: string;
}

export interface ObjectHashTable {
    add(...values: any[]): ObjectHashTable;

    remove(...values: any[]): ObjectHashTable;
    ...
    table(): { [ key: string ]: ObjectHashTableItem };

    toArray(): ObjectHashTableItemWithKey[];

    reset(): ObjectHashTable;
}

export interface ObjectHashTableStatic {
    (options?: ObjectHashOptions): ObjectHashTable;
}

export interface ObjectHash {
    (object: any, options?: ObjectHashOptions): string;
    ...
    HashTable: ObjectHashTableStatic;
}

function objectHash(object, options: ObjectHashOptions) {
...
function typeHasher(options, writeTo, context=[]) :any{
    var write = function (str:any):any {
        if ( writeTo.update )
    ...
    return {
        ...
        _object           : function (object) {
            ...
            var objType:any   = pattern.exec(objString);
